...
export default function AuthDialog({ children, type: initialType = "login" }) {
  ...
  useEffect(() => {
    ...
    console.log("Initial signup_token from cookies on mount:", token);
    if (token) {
  ...
  useEffect(() => {
    if (signupToken) {
      ...
      console.log("Signup token synced to cookies:", signupToken);
    }
  }, [signupToken]);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Save all tokens and expiry times
      Cookies.set("access_token", response.access_token, {
        expires: new Date(response.access_expiry * 1000),
        path: "/",
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
      });
      Cookies.set("access_expiry", response.access_expiry, {
        expires: new Date(response.access_expiry * 1000),
        path: "/",
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
      ...
      Cookies.set("refresh_token", response.refresh_token, {
        expires: new Date(response.refresh_expiry * 1000),
        path: "/",
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
      });
      Cookies.set("refresh_expiry", response.refresh_expiry, {
        expires: new Date(response.refresh_expiry * 1000),
        path: "/",
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Save all tokens and expiry times after OTP verification
      Cookies.set("access_token", response.access_token || token, {
        expires: new Date(response.access_expiry * 1000),
        path: "/",
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
      ...
      if (error.message === "Signup session expired. Please sign up again.") {
        setIsVerifyingOtp(false);
        setCurrentStep(1);
      }
    } finally {
